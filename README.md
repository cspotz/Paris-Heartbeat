...
This first try shows that adding the district type improves performance noticeably, while temperature had little effect. It makes sense as except for the heavy rainfall of 10th september, the weather was pretty uniform during the time frame I fetched the Vélib data. 
...
The hyperparameters left fixed in the previous section are now varied using ``optuna`` which allows for an optimal search of the ideal parameters. After 60 trials, it selected :  ``n_estimators=711,  learning_rate=0.28,  max_depth=8,
  subsample=0.58,  colsample_bytree=0.77`` as the <u> best model </u> with **RMSE=54.8** and **R²=0.96**.This configuration pushes the model toward maximum predictive performance. Deeper trees and more boosting rounds increase the risk of overfitting; however, the low learning rate and subsampling provide some regularization to mitigate this risk.
